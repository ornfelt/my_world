...
double g_range_frequency = RANGE_FREQUENCY;
...
void mod(std::vector<double> &in, std::vector<double> &out)
{
	...
	{
		out[i] = sin(pos);
		pos += M_PI * 2 * (g_base_frequency + in[i] * g_range_frequency) / double(in.size());
	}
}
...
void dem(std::vector<double> &in, std::vector<double> &out)
{
	...
	for (size_t i = 0; i < in.size(); ++i)
	{
		...
		a -= tmp;
		a = fabs(a);
		a -= g_base_frequency / double(in.size()) * 2;
		a *= in.size() * 1. / g_range_frequency / 2;
		out[i] = a;
	}
}
...
{
	if (g_range_frequency > g_base_frequency)
	{
		glColor4f(1, 0, 0, .1);
		glBegin(GL_QUADS);
		glVertex2f(0, 0);
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	std::cout << "range frequency: " << g_range_frequency << std::endl;
	run();
	(void)mods;
	(void)window;
	(void)scancode;
