...
static int read_file_data(struct pem_read_ctx *ctx, char *line, int line_len)
{
	...
	if (ctx->evp_cipher)
	{
		...
		if (!key
		 || ctx->salt_iv_len != (int)JKSSL_MAX(8, evp_cipher_get_iv_length(ctx->evp_cipher))
		 || (!ctx->password && !(ctx->password = ask_password()))
		 || !pem_get_key(ctx->evp_cipher, key, ctx->salt_iv, ctx->password))
			goto end;
