...
static const uint8_t lookup3[256] =
{
...
static const uint8_t lookup9[256] =
{
...
static const uint8_t lookup11[256] =
{
...
static const uint8_t lookup13[256] =
{
...
static const uint8_t lookup14[256] =
{
...
static void mixcolumns(uint8_t *block)
{
	...
	{
		size_t tmp = i * 4;
		*(uint32_t*)a = *(uint32_t*)(block + tmp);
		block[tmp + 0] = lookup2[a[0]] ^ a[3] ^ a[2] ^ lookup3[a[1]];
		block[tmp + 1] = lookup2[a[1]] ^ a[0] ^ a[3] ^ lookup3[a[2]];
		block[tmp + 2] = lookup2[a[2]] ^ a[1] ^ a[0] ^ lookup3[a[3]];
		block[tmp + 3] = lookup2[a[3]] ^ a[2] ^ a[1] ^ lookup3[a[0]];
	}
}

static void invmixcolumns(uint8_t *block)
{
	uint8_t a[4];
	for (size_t i = 0; i < 4; ++i)
	{
		size_t tmp = i * 4;
		*(uint32_t*)a = *(uint32_t*)(block + tmp);
		block[tmp + 0] = lookup14[a[0]] ^ lookup9[a[3]]
		               ^ lookup13[a[2]] ^ lookup11[a[1]];
		block[tmp + 1] = lookup14[a[1]] ^ lookup9[a[0]]
		               ^ lookup13[a[3]] ^ lookup11[a[2]];
		block[tmp + 2] = lookup14[a[2]] ^ lookup9[a[1]]
		               ^ lookup13[a[0]] ^ lookup11[a[3]];
		block[tmp + 3] = lookup14[a[3]] ^ lookup9[a[2]]
		               ^ lookup13[a[1]] ^ lookup11[a[0]];
	}
}
#endif
...
void aes_encrypt(struct aes_ctx *ctx, uint8_t *out, const uint8_t *in)
{
	...
	b = _mm_loadu_si128((__m128i*)in);
	b = _mm_xor_si128(b, *(__m128i*)ctx->keys[0]);
	for (size_t round = 1; round < rounds; ++round)
		b = _mm_aesenc_si128(b, ((__m128i*)ctx->keys)[round]);
	b = _mm_aesenclast_si128(b, ((__m128i*)ctx->keys)[rounds]);
	_mm_storeu_si128((__m128i*)out, b);
#else
	size_t rounds = get_rounds_count(ctx);
	uint8_t block[16];
	for (size_t i = 0; i < 16; ++i)
		block[i] = in[i] ^ ctx->keys[0][i];
	for (size_t round = 1; round < rounds; ++round)
	{
		for (size_t i = 0; i < 16; ++i)
		...
		mixcolumns(block);
		for (size_t i = 0; i < 16; ++i)
			block[i] ^= ctx->keys[round][i];
	}
	for (size_t i = 0; i < 16; ++i)
		block[i] = sbox[block[i]];
	shiftrows(block);
	for (size_t i = 0; i < 16; ++i)
		out[i] = block[i] ^ ctx->keys[rounds][i];
#endif
}
...
void aes_decrypt(struct aes_ctx *ctx, uint8_t *out, const uint8_t *in)
{
	...
	size_t rounds = get_rounds_count(ctx);
	b = _mm_loadu_si128((__m128i*)in);
	b = _mm_xor_si128(b, *(__m128i*)ctx->keys[15]);
	for (size_t round = 1; round < rounds; ++round)
		b = _mm_aesdec_si128(b, ((__m128i*)ctx->keys)[15 + round]);
	b = _mm_aesdeclast_si128(b, ((__m128i*)ctx->keys)[15 + rounds]);
	_mm_storeu_si128((__m128i*)out, b);
#else
	size_t rounds = get_rounds_count(ctx);
	uint8_t block[16];
	for (size_t i = 0; i < 16; ++i)
		block[i] = in[i] ^ ctx->keys[rounds][i];
	for (size_t round = 1; round < rounds; ++round)
	{
		invshiftrows(block);
		for (size_t i = 0; i < 16; ++i)
		...
		for (size_t i = 0; i < 16; ++i)
			block[i] ^= ctx->keys[rounds - round][i];
		invmixcolumns(block);
	}
	invshiftrows(block);
	for (size_t i = 0; i < 16; ++i)
		block[i] = inv_sbox[block[i]];
	for (size_t i = 0; i < 16; ++i)
		out[i] = block[i] ^ ctx->keys[0][i];
#endif
}

static void expand_core(uint8_t *data, uint8_t i)
{
	*(uint32_t*)data = ror32(*(uint32_t*)data, 8);
	data[0] = sbox[data[0]];
	data[1] = sbox[data[1]];
	data[2] = sbox[data[2]];
	data[3] = sbox[data[3]];
...
void aes_keyschedule(struct aes_ctx *ctx, const uint8_t *key, uint8_t len)
{
	...
	memcpy(ctx->keys[1], key + 16, len - 16);
	uint8_t i = 1;
	for (size_t c = len; c < 240;)
	{
		tmp[0] = ctx->keys[(c + 0 - 4) / 16][(c + 0 - 4) % 16];
		tmp[1] = ctx->keys[(c + 1 - 4) / 16][(c + 1 - 4) % 16];
		tmp[2] = ctx->keys[(c + 2 - 4) / 16][(c + 2 - 4) % 16];
		tmp[3] = ctx->keys[(c + 3 - 4) / 16][(c + 3 - 4) % 16];
		if (c % len == 0)
		{
			expand_core(tmp, i);
			i++;
