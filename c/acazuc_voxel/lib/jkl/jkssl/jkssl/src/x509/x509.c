...
int x509_print(struct bio *bio, struct x509 *x509)
{
	...
	bio_printf(bio, "            Public Key Algorithm: %s\n",
	           oid2txt(x509->pubkey->tbs_sigalg->oid,
...
{
	return calloc(sizeof(struct x509_pubkey), 1);
}
...
void x509_pubkey_free(struct x509_pubkey *pubkey)
{
	if (!pubkey)
		return;
	x509_algor_free(pubkey->tbs_sigalg);
	asn1_bit_string_free(pubkey->pubkey);
	free(pubkey);
}
