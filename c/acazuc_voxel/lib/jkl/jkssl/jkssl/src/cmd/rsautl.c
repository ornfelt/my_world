...
{
	ssize_t len = rsa_size(data->rsa_ctx);
	if (len == -1)
	{
...
static int handle_inform_outform(const char *arg, enum rsautl_format *format)
{
	if (!strcmp(arg, "PEM"))
	{
		*format = RSAUTL_FORMAT_PEM;
		return 1;
	}
	if (!strcmp(arg, "DER"))
	{
		*format = RSAUTL_FORMAT_DER;
		return 1;
	}
	fprintf(stderr, "rsautl: unsupported format: %s\n", arg);
	return 0;
}
	...
	while ((c = getopt_long_only(argc, argv, "", opts, NULL)) != -1)
	{
		switch (c)
		{
			...
			case 'i':
				...
				if (!data->fpin)
				{
					fprintf(stderr, "rsautl: open(%s): %s\n",
			...
			case 'x':
				...
				break;
			case 'K':
				if (!handle_inform_outform(optarg, &data->keyform))
					return 0;
				break;
			case 'k':
				data->fpkey = fopen(optarg, "r");
				if (!data->fpkey)
				{
					fprintf(stderr, "rsautl: open(%s): %s\n",
					        optarg, strerror(errno));
