	...
	while ((c = getopt_long_only(argc, argv, "", opts, NULL)) != -1)
	{
		switch (c)
		{
			...
			case 'o':
				...
				if (!data->fpout)
				{
					fprintf(stderr, "genrsa: open(%s): %s\n",
...
int cmd_genrsa(int argc, char **argv)
{
	...
	if (!rsa_generate_key(data.rsa_ctx, data.key_len, data.exp, data.gencb))
	{
		fprintf(stderr, "genrsa: failed to generate key\n");
		goto end;
	}
	fprintf(stderr, "e is ");
	bignum_print(data.rsa_ctx->e, stderr);
	fprintf(stderr, " (0x");
	bignum_printhex(data.rsa_ctx->e, stderr);
	fprintf(stderr, ")\n");
	if (!pem_write_rsa_private_key(data.fpout, data.rsa_ctx, data.cipher, data.passout))
	{
