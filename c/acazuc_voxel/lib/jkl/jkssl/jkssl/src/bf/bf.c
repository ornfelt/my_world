...
static const uint32_t s_init[0x400] =
{
...
static uint32_t bf_f(struct bf_ctx *ctx, uint32_t x)
{
...
void bf_keyschedule(struct bf_ctx *ctx, const uint8_t *key, uint8_t len)
{
	...
	{
		bf_encrypt(ctx, d, d);
		ctx->p[i + 0] = be32dec(d + 0);
		ctx->p[i + 1] = be32dec(d + 4);
	}
	for (size_t i = 0; i < 4; ++i)
	{
		...
		{
			bf_encrypt(ctx, d, d);
			ctx->s[i][j + 0] = be32dec(d + 0);
			ctx->s[i][j + 1] = be32dec(d + 4);
		}
	}
}
