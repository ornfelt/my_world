...
static const struct block_def_vtable leaves_vtable =
{
	.draw = draw,
...
{
	struct block_base *leaves = block_base_new(blocks, id, name);
	if (!leaves)
		return NULL;
	leaves->block.vtable = &leaves_vtable;
	return leaves;
}
