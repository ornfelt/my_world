...
static void pigman_draw(struct entity *entity)
{
	entity_priv_draw(entity);
	texture_bind(&g_voxel->textures->pigman);
	float t = g_voxel->frametime / 1000000000.0;
	struct pigman_def *def = &g_voxel->entities->pigman;
	VEC3_SET(def->arm_l->rot, 0, 0,  sin(t * 1) * 0.05 + 0.05);
	VEC3_SET(def->arm_r->rot, 0, 0, -sin(t * 1) * 0.05 - 0.05);
...
static const struct entity_vtable pigman_vtable =
{
	.draw = pigman_draw,
};
...
{
	struct pigman *pigman = malloc(sizeof(*pigman));
	if (!pigman)
		return NULL;
	entity_init(&pigman->entity, world, chunk);
	pigman->entity.vtable = &pigman_vtable;
	entity_set_size(&pigman->entity, (struct vec3f){0.6, 1.8, 0.6});
	return &pigman->entity;
}
