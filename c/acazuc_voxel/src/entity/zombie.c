...
static void zombie_draw(struct entity *entity)
{
	entity_priv_draw(entity);
	texture_bind(&g_voxel->textures->zombie);
	float t = g_voxel->frametime / 1000000000.0;
	struct zombie_def *def = &g_voxel->entities->zombie;
...
static const struct entity_vtable zombie_vtable =
{
	.draw = zombie_draw,
};
...
{
	struct zombie *zombie = malloc(sizeof(*zombie));
	if (!zombie)
		return NULL;
	entity_init(&zombie->entity, world, chunk);
	zombie->entity.vtable = &zombie_vtable;
	entity_set_size(&zombie->entity, (struct vec3f){0.6, 1.8, 0.6});
