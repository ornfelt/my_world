...
void
register_req_handlers(struct xsrv *xsrv)
{
	xsrv->req_handlers[CREATE_WINDOW]            = req_create_window;
	xsrv->req_handlers[CHANGE_WINDOW_ATTRIBUTES] = req_change_window_attributes;
	xsrv->req_handlers[GET_WINDOW_ATTRIBUTES]    = req_get_window_attributes;
	xsrv->req_handlers[DESTROY_WINDOW]           = req_destroy_window;
	xsrv->req_handlers[DESTROY_SUBWINDOWS]       = req_destroy_subwindows;
	xsrv->req_handlers[REPARENT_WINDOW]          = req_reparent_window;
	xsrv->req_handlers[MAP_WINDOW]               = req_map_window;
	xsrv->req_handlers[MAP_SUBWINDOWS]           = req_map_subwindows;
	xsrv->req_handlers[UNMAP_WINDOW]             = req_unmap_window;
	xsrv->req_handlers[UNMAP_SUBWINDOWS]         = req_unmap_subwindows;
	xsrv->req_handlers[CONFIGURE_WINDOW]         = req_configure_window;
	xsrv->req_handlers[GET_GEOMETRY]             = req_get_geometry;
	xsrv->req_handlers[QUERY_TREE]               = req_query_tree;
	xsrv->req_handlers[INTERN_ATOM]              = req_intern_atom;
	xsrv->req_handlers[GET_ATOM_NAME]            = req_get_atom_name;
	xsrv->req_handlers[CHANGE_PROPERTY]          = req_change_property;
	xsrv->req_handlers[DELETE_PROPERTY]          = req_delete_property;
	xsrv->req_handlers[GET_PROPERTY]             = req_get_property;
	xsrv->req_handlers[LIST_PROPERTIES]          = req_list_properties;
	xsrv->req_handlers[GRAB_POINTER]             = req_grab_pointer;
	xsrv->req_handlers[UNGRAB_POINTER]           = req_ungrab_pointer;
	xsrv->req_handlers[GRAB_BUTTON]              = req_grab_button;
	xsrv->req_handlers[UNGRAB_BUTTON]            = req_ungrab_button;
	xsrv->req_handlers[GRAB_SERVER]              = req_grab_server;
	xsrv->req_handlers[UNGRAB_SERVER]            = req_ungrab_server;
	xsrv->req_handlers[QUERY_POINTER]            = req_query_pointer;
	xsrv->req_handlers[WARP_POINTER]             = req_warp_pointer;
	xsrv->req_handlers[SET_INPUT_FOCUS]          = req_set_input_focus;
	xsrv->req_handlers[GET_INPUT_FOCUS]          = req_get_input_focus;
	xsrv->req_handlers[OPEN_FONT]                = req_open_font;
	xsrv->req_handlers[CLOSE_FONT]               = req_close_font;
	xsrv->req_handlers[LIST_FONTS]               = req_list_fonts;
	xsrv->req_handlers[CREATE_PIXMAP]            = req_create_pixmap;
	xsrv->req_handlers[FREE_PIXMAP]              = req_free_pixmap;
	xsrv->req_handlers[CREATE_GC]                = req_create_gc;
	xsrv->req_handlers[CHANGE_GC]                = req_change_gc;
	xsrv->req_handlers[COPY_GC]                  = req_copy_gc;
	xsrv->req_handlers[FREE_GC]                  = req_free_gc;
	xsrv->req_handlers[CLEAR_AREA]               = req_clear_area;
	xsrv->req_handlers[COPY_AREA]                = req_copy_area;
	xsrv->req_handlers[POLY_POINT]               = req_poly_point;
	xsrv->req_handlers[POLY_LINE]                = req_poly_line;
	xsrv->req_handlers[POLY_SEGMENT]             = req_poly_segment;
	xsrv->req_handlers[POLY_RECTANGLE]           = req_poly_rectangle;
	xsrv->req_handlers[POLY_ARC]                 = req_poly_arc;
	xsrv->req_handlers[FILL_POLY]                = req_fill_poly;
	xsrv->req_handlers[POLY_FILL_RECTANGLE]      = req_poly_fill_rectangle;
	xsrv->req_handlers[POLY_FILL_ARC]            = req_poly_fill_arc;
	xsrv->req_handlers[PUT_IMAGE]                = req_put_image;
	xsrv->req_handlers[GET_IMAGE]                = req_get_image;
	xsrv->req_handlers[IMAGE_TEXT8]              = req_image_text8;
	xsrv->req_handlers[CREATE_CURSOR]            = req_create_cursor;
	xsrv->req_handlers[CREATE_GLYPH_CURSOR]      = req_create_glyph_cursor;
	xsrv->req_handlers[FREE_CURSOR]              = req_free_cursor;
	xsrv->req_handlers[RECOLOR_CURSOR]           = req_recolor_cursor;
	xsrv->req_handlers[QUERY_BEST_SIZE]          = req_query_best_size;
	xsrv->req_handlers[QUERY_EXTENSION]          = req_query_extension;
	xsrv->req_handlers[LIST_EXTENSIONS]          = req_list_extensions;
	xsrv->req_handlers[CHANGE_KEYBOARD_MAPPING]  = req_change_keyboard_mapping;
	xsrv->req_handlers[GET_KEYBOARD_MAPPING]     = req_get_keyboard_mapping;
	xsrv->req_handlers[CHANGE_KEYBOARD_CONTROL]  = req_change_keyboard_control;
	xsrv->req_handlers[GET_KEYBOARD_CONTROL]     = req_get_keyboard_control;
	xsrv->req_handlers[BELL]                     = req_bell;
	xsrv->req_handlers[CHANGE_POINTER_CONTROL]   = req_change_pointer_control;
	xsrv->req_handlers[GET_POINTER_CONTROL]      = req_get_pointer_control;
	xsrv->req_handlers[SET_POINTER_MAPPING]      = req_set_pointer_mapping;
	xsrv->req_handlers[GET_POINTER_MAPPING]      = req_get_pointer_mapping;
	xsrv->req_handlers[SET_MODIFIER_MAPPING]     = req_set_modifier_mapping;
	xsrv->req_handlers[GET_MODIFIER_MAPPING]     = req_get_modifier_mapping;
	xsrv->req_handlers[NO_OPERATION]             = req_no_operation;
}
