...
static const struct object_def pixmap_def =
{
	.name = "pixmap",
	.free = pixmap_free,
};
...
struct pixmap *pixmap_new(struct xsrv *xsrv, struct client *client, uint32_t id,
                          uint16_t width, uint16_t height,
                          const struct format *format, struct window *root)
{
	struct pixmap *pixmap = malloc(sizeof(*pixmap));
	if (!pixmap)
	{
