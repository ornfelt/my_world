...
int backend_init(struct xsrv *xsrv)
{
	...
	fb0 = mmap(NULL, fb_info.height * fb_info.pitch,
	           PROT_READ | PROT_WRITE, MAP_PRIVATE, fb, 0);
	if (fb0 == MAP_FAILED)
	{
	...
	xsrv->backend.data = mmap(NULL, fb_info.height * fb_info.pitch,
	...
	{
		fprintf(stderr, "%s: open(%s): %s\n", xsrv->progname,
		        "/dev/event0", strerror(errno));
		return 1;
	}

	int flags = fcntl(xsrv->backend.fd, F_GETFL, 0);
	if (flags < 0)
	{
...
static void commit_pointer(struct xsrv *xsrv)
{
...
static void process_pointer_event(struct xsrv *xsrv,
...
static void process_mouse_event(struct xsrv *xsrv, struct mouse_event *event)
{
	if (event->pressed)
...
static void process_scroll_event(struct xsrv *xsrv, struct scroll_event *event)
{
	if (event->y < 0)
	{
...
static void process_key_event(struct xsrv *xsrv, struct key_event *event)
{
	if (event->pressed)
...
static void process_event(struct xsrv *xsrv, struct event *event)
{
	switch (event->type)
	{
		case EVENT_POINTER:
...
int backend_poll(struct xsrv *xsrv)
{
	...
	ssize_t rd = read(xsrv->backend.fd, &events, sizeof(events));
	if (rd == -1 && errno == EAGAIN)
		return 0;
	if (rd <= 0)
		return 1;
	size_t nevents = (size_t)rd / sizeof(*events);
...
static void setup_cursor(void)
{
	...
	Pixmap source = XCreateBitmapFromData(display, window, zero, 32, 16);
	Pixmap mask = XCreateBitmapFromData(display, window, zero, 32, 16);
	XColor background;
...
int backend_init(struct xsrv *xsrv)
{
	...
	image = XShmCreateImage(display, vi->visual, 24, ZPixmap, NULL,
	...
	shminfo.shmid = shmget(387920, image->bytes_per_line * image->height,
	                       IPC_CREAT | 0777);
	if (shminfo.shmid == -1)
	{
...
{
	(void)xsrv;
	XShmPutImage(display, window, gc, image, rect->left, rect->top,
