...
Bool
XRenderQueryExtension(Display *display,
                      int *event_base,
                      int *error_base)
{
	int major_opcode;
...
Status
XRenderQueryVersion(Display *display,
                    int *major,
                    int *minor)
{
...
Status
XRenderQueryFormats(Display *display)
{
	XRenderPictFormat *formats;
	...
	if (!reply)
		return False;
	formats_count = xcb_render_query_pict_formats_formats_length(reply);
	formats = malloc(sizeof(*formats) * formats_count);
	if (!formats)
	{
		free(reply);
		return False;
...
XRenderPictFormat *
XRenderFindFormat(Display *display,
                  unsigned long mask,
...
XRenderPictFormat *
XRenderFindVisualFormat(Display *display,
                        const Visual *visual)
{
...
XRenderPictFormat *
XRenderFindStandardFormat(Display *display,
                          int format)
{
	XRenderPictFormat templ;
	unsigned long mask;
...
Picture
XRenderCreatePicture(Display *display,
                     Drawable drawable,
