...
static void
resize(struct render_pass *render_pass,
       uint32_t width,
       uint32_t height)
{
...
const struct render_pass_vtable
render_pass_vtable =
{
	.ctr     = ctr,
	.dtr     = dtr,
...
void
render_pass_process(struct render_pass *render_pass,
                    struct render_target *src,
                    struct render_target *dst,
                    uint32_t buffers)
{
...
void
render_pass_resize(struct render_pass *render_pass,
                   uint32_t width,
                   uint32_t height)
{
	if (render_pass->width == width
	 && render_pass->height == height)
		return;
...
void
render_pass_delete(struct render_pass *render_pass)
{
	if (!render_pass)
		return;
