...
static const struct vec4f light_direction = {-1.0f, -1.5f, 1.0f, 0.0f};

static void list_init(struct gx_frame_list *list, size_t element_size)
{
	jks_array_init(&list->entries, element_size, NULL, &jks_array_memory_fn_GX);
	pthread_mutex_init(&list->mutex, NULL);
}

static void list_destroy(struct gx_frame_list *list)
{
	jks_array_destroy(&list->entries);
	pthread_mutex_destroy(&list->mutex);
}

static void list_clear(struct gx_frame_list *list)
{
...
static void list_push(struct gx_frame_list *list, void *entry)
{
	pthread_mutex_lock(&list->mutex);
...
void gx_frame_init(struct gx_frame *frame, int id)
{
	...
	gfx_create_buffer(g_wow->device, &frame->m2_ground_shadow_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_m2_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->m2_ground_light_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_m2_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->m2_world_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_m2_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->particle_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_particle_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->ribbon_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_ribbon_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->river_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_mclq_water_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->ocean_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_mclq_water_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->magma_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_mclq_magma_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->mcnk_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_mcnk_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->mliq_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_mliq_scene_block), GFX_BUFFER_STREAM);
	gfx_create_buffer(g_wow->device, &frame->wmo_uniform_buffer, GFX_BUFFER_UNIFORM, NULL, sizeof(struct shader_wmo_scene_block), GFX_BUFFER_STREAM);
#ifdef WITH_DEBUG_RENDERING
...
void gx_frame_destroy(struct gx_frame *frame)
{
#ifdef WITH_DEBUG_RENDERING
...
static void build_particle_uniform_buffer(struct gx_frame *frame)
{
...
static void build_ribbon_uniform_buffer(struct gx_frame *frame)
{
...
static void build_ocean_uniform_buffer(struct gx_frame *frame)
{
...
static void build_river_uniform_buffer(struct gx_frame *frame)
{
	struct shader_mclq_water_scene_block scene_block;
	VEC4_CPY(scene_block.light_direction, light_direction);
	COPY_VEC3_A1(scene_block.specular_color, g_wow->map->gx_skybox->int_values[SKYBOX_INT_SUN]);
...
static void build_magma_uniform_buffer(struct gx_frame *frame)
{
...
static void build_mcnk_uniform_buffer(struct gx_frame *frame)
{
...
static void build_mliq_uniform_buffer(struct gx_frame *frame)
{
...
static void build_wmo_uniform_buffer(struct gx_frame *frame)
{
...
static void build_m2_ground_shadow_uniform_buffer(struct gx_frame *frame)
{
...
static void build_m2_ground_light_uniform_buffer(struct gx_frame *frame)
{
	struct shader_m2_scene_block scene_block;
	VEC4_CPY(scene_block.light_direction, light_direction);
	VEC4_SET(scene_block.specular_color, 0, 0, 0, 0);
...
static void build_m2_world_uniform_buffer(struct gx_frame *frame)
{
	struct shader_m2_scene_block scene_block;
	VEC4_CPY(scene_block.light_direction, light_direction);
	COPY_VEC3_A1(scene_block.specular_color, g_wow->map->gx_skybox->int_values[SKYBOX_INT_SUN]);
...
void gx_frame_build_uniform_buffers(struct gx_frame *frame)
{
...
void gx_frame_copy_cameras(struct gx_frame *frame, struct camera *cull_camera, struct camera *view_camera)
{
...
void gx_frame_clear_scene(struct gx_frame *frame)
{
...
void gx_frame_release_obj(struct gx_frame *frame)
{
	for (size_t i = 0; i < frame->backrefs.tiles.entries.size; ++i)
	{
		struct map_tile *tile = *JKS_ARRAY_GET(&frame->backrefs.tiles.entries, i, struct map_tile*);
