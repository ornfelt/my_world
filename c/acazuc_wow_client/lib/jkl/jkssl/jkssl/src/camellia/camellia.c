...
static uint64_t f(uint64_t in1, uint64_t in2)
{
	uint8_t t[8];
	t[0] = sbox1[((in1 >> 56) ^ (in2 >> 56)) & 0xff];
	t[1] = sbox2[((in1 >> 48) ^ (in2 >> 48)) & 0xff];
	t[2] = sbox3[((in1 >> 40) ^ (in2 >> 40)) & 0xff];
	t[3] = sbox4[((in1 >> 32) ^ (in2 >> 32)) & 0xff];
	t[4] = sbox2[((in1 >> 24) ^ (in2 >> 24)) & 0xff];
	t[5] = sbox3[((in1 >> 16) ^ (in2 >> 16)) & 0xff];
	t[6] = sbox4[((in1 >>  8) ^ (in2 >>  8)) & 0xff];
	t[7] = sbox1[((in1 >>  0) ^ (in2 >>  0)) & 0xff];
	uint64_t out = 0;
...
{
	*(uint64_t*)(klr + 0) = bswap64(*(const uint64_t*)(key + 0));
	*(uint64_t*)(klr + 8) = bswap64(*(const uint64_t*)(key + 8));
	if (len == 16)
	{
		memset(klr + 16, 0, 16);
	}
	...
	{
		*(uint64_t*)(klr + 16) = bswap64(*(const uint64_t*)(key + 16));
		*(uint64_t*)(klr + 24) = ~bswap64(*(const uint64_t*)(key + 16));
	}
	else
	{
		*(uint64_t*)(klr + 16) = bswap64(*(const uint64_t*)(key + 16));
		*(uint64_t*)(klr + 24) = bswap64(*(const uint64_t*)(key + 24));
	}
}

static void fill_ka(uint8_t *kab, uint8_t *klr)
{
	uint64_t d1 = *(uint64_t*)(klr + 0) ^ *(uint64_t*)(klr + 16);
	uint64_t d2 = *(uint64_t*)(klr + 8) ^ *(uint64_t*)(klr + 24);
	d2 ^= f(d1, sigma1);
	d1 ^= f(d2, sigma2);
	d1 ^= *(uint64_t*)(klr + 0);
	d2 ^= *(uint64_t*)(klr + 8);
...
{
	uint64_t d1 = *(uint64_t*)(kab + 0) ^ *(uint64_t*)(klr + 16);
	uint64_t d2 = *(uint64_t*)(kab + 8) ^ *(uint64_t*)(klr + 24);
	d2 ^= f(d1, sigma5);
	d1 ^= f(d2, sigma6);
	memcpy(kab + 16, &d1, 8);
	memcpy(kab + 24, &d2, 8);
