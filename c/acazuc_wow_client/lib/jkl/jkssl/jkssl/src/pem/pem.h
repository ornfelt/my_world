...
#define PEM_WRITE_BIO(name, txt, ctx, cipher, pass) \
do \
{ \
...
#define PEM_READ_BIO(name, ctx_t, txt, ctx, pass) \
do \
{ \
...
{ \
	PEM_READ_BIO(name, struct st, txt, v, NULL); \
} \
...
int pem_write_bio_##name(struct bio *bio, struct st *v, \
                                ...
                                const char *pass) \
{ \
	PEM_WRITE_BIO(name, txt, v, cipher, pass); \
} \
int pem_write_##name(FILE *fp, struct st *v, \
                            const struct evp_cipher *cipher, \
                            const char *pass) \
{ \
	PEM_WRITE_FP(name, v, cipher, pass); \
}
