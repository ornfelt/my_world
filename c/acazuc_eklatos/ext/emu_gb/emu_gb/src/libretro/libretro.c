...
bool retro_load_game(const struct retro_game_info *info)
{
	...
	if (!load_bios("dmg_bios.bin", dmg_bios, sizeof(dmg_bios)))
	{
		log_cb(RETRO_LOG_ERROR, "failed to load dmg_bios.bin\n");
		goto err;
	}
	if (!load_bios("cgb_bios.bin", cgb_bios, sizeof(cgb_bios)))
	{
		log_cb(RETRO_LOG_ERROR, "failed to load cgb_bios.bin\n");
		goto err;
	}
	gb_set_dmg_bios(g_gb, dmg_bios);
	gb_set_cgb_bios(g_gb, cgb_bios);
	return true;
