...
static void print_any(XAnyEvent *e, const char *name)
{
	printf("%s event, serial %lu, synthetic %s window 0x%x\n", name, e->serial,
	       BOOL_STR(e->send_event), e->window);
}

static void print_key(XKeyEvent *e)
{
	...
	printf("    state 0x%x, keycode %u (keysym 0x%x, %c), same_screen %s\n",
	       e->state, e->keycode, sym, (sym < 0x80 && isprint(sym)) ? sym : ' ',
	       BOOL_STR(e->same_screen));
}

static void print_button(XButtonEvent *e)
{
	printf("    root 0x%x, subw 0x%x, time %u, (%d,%d), root:(%d,%d)\n",
	       e->root, e->subwindow, e->time, e->x, e->y, e->x_root, e->y_root);
...
static void print_motion(XMotionEvent *e)
{
	printf("    root 0x%x, subw 0x%x, time %u, (%d,%d), root:(%d,%d)\n",
	       e->root, e->subwindow, e->time, e->x, e->y, e->x_root, e->y_root);
...
static const char *notify_details[] =
{
...
static const char *notify_modes[] =
{
...
static void print_crossing(XCrossingEvent *e)
{
	printf("    root 0x%x, subw 0x%x, time %u, (%d,%d), root:(%d,%d)\n",
	       e->root, e->subwindow, e->time, e->x, e->y, e->x_root, e->y_root);
	printf("    mode %s, detail %s, same_screen %s\n", notify_modes[e->mode],
	       notify_details[e->detail], BOOL_STR(e->same_screen));
	printf("    focus %s, state %x\n", BOOL_STR(e->focus), e->state);
}

static void print_focus(XFocusChangeEvent *e)
{
	printf("    mode %s, detail %s\n", notify_modes[e->mode],
	       notify_details[e->detail]);
}

static void print_keymap(XKeymapEvent *e)
{
	printf("    keys:  ");
	for (int i = 0; i < 16; ++i)
...
static void print_expose(XExposeEvent *e)
{
	printf("    (%d,%d), width %d, height %d, count %d\n", e->x, e->y,
	       e->width, e->height, e->count);
}

static void print_graphics_expose(XGraphicsExposeEvent *e)
{
	printf("    (%d,%d), width %d, height %d, count %d\n", e->x, e->y,
	       e->width, e->height, e->count);
...
static void print_no_expose(XNoExposeEvent *e)
{
	printf("    major %d, minor %d\n", e->major_code, e->minor_code);
}

static void print_visibility(XVisibilityEvent *e)
{
	printf("    state %d\n", e->state);
}

static void print_create(XCreateWindowEvent *e)
{
	printf("    (%d,%d), width %d, height %d\n", e->x, e->y, e->width,
	       e->height);
	printf("    border_width %d, above 0x%x, override %s\n", e->border_width,
	       e->parent, BOOL_STR(e->override_redirect));
}

static void print_destroy(XDestroyWindowEvent *e)
{
	printf("    event 0x%x, window 0x%x\n", e->event, e->window);
}

static void print_unmap(XUnmapEvent *e)
{
	printf("    event 0x%x, window 0x%x, from_configure %s\n", e->event,
	       e->window, BOOL_STR(e->from_configure));
}

static void print_map(XMapEvent *e)
{
	printf("    event 0x%x, window 0x%x, override %s\n", e->event, e->window,
	       BOOL_STR(e->override_redirect));
}

static void print_map_request(XMapRequestEvent *e)
{
	printf("    parent 0x%x\n", e->parent);
}

static void print_reparent(XReparentEvent *e)
{
	printf("    (%d,%d), override %s\n", e->x, e->y,
	       BOOL_STR(e->override_redirect));
}

static void print_configure(XConfigureEvent *e)
{
	printf("    (%d,%d), width %d, height %d\n", e->x, e->y, e->width,
	       e->height);
	printf("    border_width %d, above 0x%x, override %s\n", e->border_width,
	       e->above, BOOL_STR(e->override_redirect));
}

static const char *stack_modes[] =
{
	[Above]    = "Above",
	[Below]    = "Below",
	[TopIf]    = "TopIf",
...
static void print_configure_request(XConfigureRequestEvent *e)
{
	printf("    (%d,%d), width %d, height %d\n", e->x, e->y, e->width,
	       e->height);
	printf("    border_width %d, above 0x%x, detail %s, value 0x%lx\n",
	       e->border_width, e->parent, stack_modes[e->detail], e->value_mask);
}

static void print_gravity(XGravityEvent *e)
{
	printf("    (%d,%d)\n", e->x, e->y);
}

static void print_resize(XResizeRequestEvent *e)
{
...
static void print_circulate(XCirculateEvent *e)
{
	printf("    event %x, window %x, place %s\n", e->event, e->window,
	       places[e->place]);
}

static void print_circulate_request(XCirculateRequestEvent *e)
{
	printf("    parent %x, window %x, place %s\n", e->parent, e->window,
	       places[e->place]);
}

static const char *property_states[] =
{
	[PropertyNewValue] = "PropertyNewValue",
	[PropertyDelete]  = "PropertyDelete",
};

static void print_property(XPropertyEvent *e)
{
	char *name = XGetAtomName(e->display, e->atom);
	printf("    atom 0x%x (%s), time %u, state %s\n", e->atom, name ? name : "",
	       e->time, property_states[e->state]);
	free(name);
}

static void print_selection_clear(XSelectionClearEvent *e)
{
	printf("    selection 0x%x, time %u\n", e->selection, e->time);
}

static void print_selection_request(XSelectionRequestEvent *e)
{
	printf("    selection 0x%x, target 0x%x, property 0x%x, time %u\n",
	       e->selection, e->target, e->property, e->time);
}

static void print_selection(XSelectionEvent *e)
{
	printf("    selection 0x%x, target 0x%x, property 0x%x, time %u\n",
	       e->selection, e->target, e->property, e->time);
}

static const char *colormap_states[] =
{
	[ColormapUninstalled] = "ColormapUninstalled",
	[ColormapInstalled]   = "ColormapInstalled",
};

static void print_colormap(XColormapEvent *e)
{
	printf("    colormap 0x%x, new %s, state %s\n", e->colormap,
	       BOOL_STR(e->new), colormap_states[e->state]);
}

static void print_client(XClientMessageEvent *e)
{
	printf("    type 0x%x, format %d\n", e->type, e->format);
}

static const char *mapping_requests[] =
{
...
static void print_mapping(XMappingEvent *e)
{
	printf("    request %s, first_keycode 0x%x, count %d\n",
	       mapping_requests[e->request], e->first_keycode, e->count);
}
