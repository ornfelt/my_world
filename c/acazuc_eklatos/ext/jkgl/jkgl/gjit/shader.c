...
int
gjit_shader_alloc(struct jkg_ctx *ctx,
...
void
gjit_shader_free(struct jkg_ctx *ctx,
                 struct jkg_shader *shader)
{
	(void)ctx;
	free(shader);
...
int
gjit_shader_state_alloc(struct jkg_ctx *ctx,
...
int
gjit_shader_state_bind(struct jkg_ctx *ctx,
                       struct jkg_shader_state *state)
{
...
void
gjit_shader_state_free(struct jkg_ctx *ctx,
                       struct jkg_shader_state *state)
{
	(void)ctx;
	free(state);
