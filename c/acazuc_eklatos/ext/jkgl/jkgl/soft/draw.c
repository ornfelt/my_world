...
static void
get_attrib(struct soft_draw_ctx *ctx,
           const struct jkg_input_layout_attrib *attrib,
           uint32_t instance,
           uint32_t index,
           struct rast_vec *vec)
{
	struct jkg_vertex_buffer *vertex_buffer;
	void *buffer_data;
	size_t offset;
	...
	vertex_buffer = &ctx->ctx->vertex_buffers[attrib->binding];
	if (attrib->divisor)
		offset = instance / attrib->divisor;
	else
		offset = index;
	offset *= vertex_buffer->stride;
	offset += attrib->offset;
	buffer_data = &((uint8_t*)vertex_buffer->buffer->data)[offset];
	jkg_get_vec4f(&vec->x, buffer_data, attrib->format);
}
         ...
         struct rast_vert *vert)
{
	...
	struct soft_draw_ctx *ctx = rast->userdata;
	...
	input_layout = ctx->ctx->input_layout;
	for (size_t i = 0; i < input_layout->nattribs; ++i)
		get_attrib(ctx, &input_layout->attribs[i], instance, index, &vert->attribs[i]);
...
int
soft_draw(struct jkg_ctx *ctx, const struct jkg_draw_cmd *cmd)
{
	struct soft_draw_ctx draw;

	for (size_t i = 0; i < SOFT_MAX_UNIFORM_BLOCKS; ++i)
		ctx->uniform_data[i] = ctx->uniform_blocks[i].buffer ? ctx->uniform_blocks[i].buffer->data : NULL;
	draw.ctx = ctx;
	draw.cmd = cmd;
	draw.rast.fragment = soft_fragment;
...
int
soft_set_viewport(struct jkg_ctx *ctx, const float * restrict viewport)
{
...
int
soft_set_scissor(struct jkg_ctx *ctx, const float * restrict scissor)
{
