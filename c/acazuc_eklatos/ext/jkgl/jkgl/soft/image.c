...
int
soft_image_alloc(struct jkg_ctx *ctx,
...
void
soft_image_free(struct jkg_ctx *ctx,
                struct jkg_image *image)
{
	(void)ctx;
	free(image->data);
...
int
soft_image_read(struct jkg_ctx *ctx,
                struct jkg_image *image,
                uint32_t level,
                void *data,
...
int
soft_image_write(struct jkg_ctx *ctx,
                 struct jkg_image *image,
                 uint32_t level,
                 const void *data,
                 enum jkg_format format,
...
int
soft_image_copy(struct jkg_ctx *ctx,
