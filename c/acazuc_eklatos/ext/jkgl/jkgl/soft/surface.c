...
int
soft_surface_alloc(struct jkg_ctx *ctx,
...
int
soft_surface_bind(struct jkg_ctx *ctx,
                  ...
                  struct jkg_surface *stencil)
{
	...
	for (uint32_t i = count; i < SOFT_MAX_ATTACHMENTS; ++i)
...
int
soft_surface_clear(struct jkg_ctx *ctx,
...
void
soft_surface_free(struct jkg_ctx *ctx,
                  struct jkg_surface *surface)
{
	(void)ctx;
	free(surface);
