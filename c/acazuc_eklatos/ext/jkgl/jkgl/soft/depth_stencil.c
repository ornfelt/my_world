...
int
soft_depth_stencil_state_alloc(struct jkg_ctx *ctx,
...
int
soft_depth_stencil_state_bind(struct jkg_ctx *ctx,
                              struct jkg_depth_stencil_state *state)
{
...
void
soft_depth_stencil_state_free(struct jkg_ctx *ctx,
                              struct jkg_depth_stencil_state *state)
{
	(void)ctx;
	free(state);
...
int
soft_set_stencil_ref(struct jkg_ctx *ctx, uint8_t back, uint8_t front)
{
