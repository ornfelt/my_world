                   ...
                   struct bignum_ctx *ctx)
{
	...
	if (__bignum_is_zero(a) || n >= (uint32_t)bignum_num_bits(a))
	{
		__bignum_zero(r);
		return 1;
	}
...
int bignum_rshift(struct bignum *r, const struct bignum *a, uint32_t n,
                  struct bignum_ctx *ctx)
{
	int negative = __bignum_is_negative(a);
	int ret = bignum_urshift(r, a, n, ctx);
	__bignum_set_negative(r, negative);
	return ret;
}
                    ...
                    struct bignum_ctx *ctx)
{
	...
	(void)ctx;
	if (__bignum_is_bit_set(a, a->len * sizeof(*a->data) * 8 - 1))
		add = 1;
	else
		add = 0;
