...
{ \
	...
	CIPHER_FOREACH(rc5_32_12_16_ecb);
	CIPHER_FOREACH(rc5_32_12_16_cbc);
	CIPHER_FOREACH(rc5_32_12_16_pcbc);
	CIPHER_FOREACH(rc5_32_12_16_cfb);
	CIPHER_FOREACH(rc5_32_12_16_ofb);
	CIPHER_FOREACH(rc5_32_12_16_ctr);
...
static int cipher_by_name_foreach(const struct evp_cipher *cipher, void *data)
{
...
const struct evp_cipher *evp_get_cipherbyname(const char *name)
{
	...
	state.name = name;
	state.cipher = NULL;
	evp_foreach_cipher(cipher_by_name_foreach, &state);
	return state.cipher;
}

static int cipher_by_oid_foreach(const struct evp_cipher *cipher, void *data)
{
	struct cipher_get_state *state = data;
	if (cipher->oid_size == state->oid_size
	 && !memcmp(cipher->oid, state->oid, state->oid_size * sizeof(uint32_t)))
	{
		state->cipher = cipher;
		return 0;
	}
...
const struct evp_cipher *evp_get_cipherbyoid(const uint32_t *oid, size_t size)
{
	struct cipher_get_state state;
	state.oid = oid;
	state.oid_size = size;
	state.cipher = NULL;
	evp_foreach_cipher(cipher_by_oid_foreach, &state);
	return state.cipher;
}
