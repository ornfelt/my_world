	...
	while ((c = getopt_long_only(argc, argv, "", opts, NULL)) != -1)
	{
		switch (c)
		{
			...
			case 's':
			{
				...
				FILE *fp = NULL;
				if (!cmd_handle_in("dgst", optarg, &fp))
					return 0;
				data->sign = pem_read_private_key(fp, NULL, NULL, 0);
				fclose(fp);
				if (!data->sign)
				{
			...
			case 'v':
			{
				...
				if (!cmd_handle_in("dgst", optarg, &fp))
					return 0;
				data->verify = pem_read_pubkey(fp, NULL, NULL, 0);
				fclose(fp);
				if (!data->verify)
				{
			...
			case 'V':
			{
				...
				if (!cmd_handle_in("dgst", optarg, &fp))
					return 0;
				data->verify = pem_read_private_key(fp, NULL, NULL, 0);
				fclose(fp);
				if (!data->verify)
				{
					fprintf(stderr, "dgst: failed to read private key\n");
			...
			case 'o':
				if (!cmd_handle_out("dgst", optarg, &data->out))
					return 0;
				break;
			case 'S':
				if (!cmd_handle_in("dgst", optarg,
				                   &data->signature))
					return 0;
				break;
			default:
