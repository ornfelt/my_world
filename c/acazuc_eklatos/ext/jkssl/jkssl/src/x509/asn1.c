...
struct asn1_object *i2a_x509_algor(const struct x509_algor *algor)
{
...
struct asn1_object *i2a_x509_name_entry(const struct x509_name_entry *entry)
{
...
struct asn1_object *i2a_x509_name(const struct x509_name *name)
{
	struct asn1_constructed *sequence = asn1_sequence_new();
	struct asn1_object *ret = NULL;
	if (!sequence)
...
struct asn1_object *i2a_x509_extension(const struct x509_extension *extension)
{
	struct asn1_constructed *sequence = asn1_sequence_new();
...
struct evp_pkey *a2i_pubkey(const struct asn1_object *object,
                            struct evp_pkey **pkeyp)
{
	...
	else if (!asn1_oid_cmp_uint32(algorithm, oid_rsaEncryption,
	...
	else if (!asn1_oid_cmp_uint32(algorithm, oid_ecPublicKey,
	                              sizeof(oid_ecPublicKey) / sizeof(*oid_ecPublicKey)))
	{
		struct ec_key *ec = ec_key_new();
		if (!ec)
			goto err;
		if (!a2i_ec_pubkey(object, &ec)
...
struct asn1_object *i2a_pubkey(const struct evp_pkey *pkey)
{
	switch (pkey->type)
	{
		case EVP_PKEY_RSA:
			return i2a_rsa_pubkey(pkey->rsa);
		case EVP_PKEY_DSA:
			return i2a_dsa_pubkey(pkey->dsa);
		case EVP_PKEY_EC:
			return i2a_ec_pubkey(pkey->ec);
		default:
			return NULL;
	}
}
                           ...
                           const struct asn1_constructed *sequence)
{
	if (sequence->objects_count < 7
	 ...
	 || !a2i_pubkey(sequence->objects[6], &x509->pkey))
		return 0;
	if (sequence->objects_count <= 7)
		return 1;
	size_t n = 7;
...
static struct asn1_object *i2a_version(const struct x509 *x509)
{
...
static struct asn1_object *i2a_validity(const struct x509 *x509)
{
	struct asn1_constructed *sequence = asn1_sequence_new();
	struct asn1_object *ret = NULL;
	if (!sequence
...
static struct asn1_object *i2a_x509_extensions(const struct x509 *x509)
{
...
static struct asn1_object *i2a_certificate(const struct x509 *x509)
{
	struct asn1_constructed *sequence = asn1_sequence_new();
	...
	struct asn1_object *subject_pki = i2a_pubkey(x509->pkey);
...
struct asn1_object *i2a_x509(const struct x509 *x509)
{
	struct asn1_constructed *sequence = asn1_sequence_new();
