...
static int fatfs_mount(struct node *dir,
                       struct node *dev,
                       unsigned long flags,
                       const void *udata,
                       struct fs_sb **sb);
...
static uint32_t
get_ino(const struct fatfs_dirent *dirent)
{
...
static mode_t
get_mode(const struct fatfs_dirent *dirent)
{
	mode_t mode;
...
static void
get_timespec(struct node *node, const struct fatfs_dirent *dirent)
{
...
static int
fs_mknode(struct fatfs_sb *fatsb,
...
static int
fatfs_node_release(struct node *node)
{
...
static int
get_fat_cache_page(struct fatfs_sb *fatsb, uint64_t off, uint8_t **blk)
{
...
static int
get_sector_offset(struct fatfs_sb *fatsb,
...
static ssize_t
node_read(struct fatfs_node *node, struct uio *uio)
{
...
static int
get_node_cache_page(struct fatfs_node *node, uint64_t off, uint8_t **blk)
{
...
static int
dir_iterate(struct fatfs_node *dir,
...
static int
lookup_cb(const struct fatfs_dirent *dirent, void *userdata)
{
...
static int
readdir_cb(const struct fatfs_dirent *dirent, void *data)
{
...
static int
update_free_blocks(struct fatfs_sb *fatsb)
{
	uint8_t carry = 0;
	uint8_t ncarry = 0;
	uint32_t sectors = 0;
...
static int
fatfs_stat(struct fs_sb *sb, struct statvfs *st)
{
