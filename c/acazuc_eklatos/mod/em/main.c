...
static inline uint32_t em_read(struct em *em, uint32_t reg)
{
	return pci_r32(em->pci_map, reg);
}

static inline void em_write(struct em *em, uint32_t reg, uint32_t val)
{
	pci_w32(em->pci_map, reg, val);
}

static void queue_lock(struct em_queue *queue)
{
	mutex_lock(&queue->mutex);
}

static void queue_unlock(struct em_queue *queue)
{
...
void update_rx_filter(struct netif *netif)
{
	struct em *em = netif->userdata;
	uint32_t rctl;
...
const struct netif_op netif_op =
{
	.emit = emit_pkt,
