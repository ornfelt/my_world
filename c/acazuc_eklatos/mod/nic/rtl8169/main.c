...
rtl_r8(struct rtl8169 *rtl, uint32_t reg)
{
	return pci_r8(rtl->pci_map, reg);
}

static inline uint16_t
rtl_r16(struct rtl8169 *rtl, uint32_t reg)
{
	return pci_r16(rtl->pci_map, reg);
}

static inline uint32_t
rtl_r32(struct rtl8169 *rtl, uint32_t reg)
{
	return pci_r32(rtl->pci_map, reg);
}

static inline void
rtl_w8(struct rtl8169 *rtl, uint32_t reg, uint8_t val)
{
	pci_w8(rtl->pci_map, reg, val);
}

static inline void
rtl_w16(struct rtl8169 *rtl, uint32_t reg, uint16_t val)
{
	pci_w16(rtl->pci_map, reg, val);
}

static inline void
rtl_w32(struct rtl8169 *rtl, uint32_t reg, uint32_t val)
{
	pci_w32(rtl->pci_map, reg, val);
}

static inline void
rtl_lock(struct rtl8169 *rtl)
{
	mutex_lock(&rtl->mutex);
}

static inline void
rtl_unlock(struct rtl8169 *rtl)
{
...
static void
rx_desc(struct rtl8169 *rtl, struct rtl8169_desc *desc, size_t i)
{
...
static int
alloc_desc(struct dma_buf **descs, struct dma_buf **bufs, int rx)
{
	int ret;
...
void
rtl8169_free(struct rtl8169 *rtl)
{
	if (!rtl)
		return;
