...
static void
outchar(struct strftime_ctx *ctx, wchar_t c)
{
	if (ctx->len < ctx->size)
		ctx->data[ctx->len] = c;
	ctx->len++;
...
static void
outstr(struct strftime_ctx *ctx, const wchar_t *s)
{
...
static void
print_twodigits(struct strftime_ctx *ctx, int n, wchar_t pad)
{
	if (n >= 10)
...
size_t
wcsftime(wchar_t *s, size_t max, const wchar_t *format, const struct tm *tm)
{
