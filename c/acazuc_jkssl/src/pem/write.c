                            ...
                            struct bio *bio)
{
	...
	for (size_t i = 0; salt_iv_text[i]; ++i)
		salt_iv_text[i] = toupper(salt_iv_text[i]);
...
static int print_ciphered(struct pem_write_ctx *ctx, struct bio *bio)
{
	...
	if (ctx->password_cb)
	{
		if (ctx->password_cb(buf, sizeof(buf), 1, ctx->password_userdata) < 0)
			goto end;
		password = buf;
	}
	...
	if (rand_bytes(salt_iv, JKSSL_MAX(8, evp_cipher_get_iv_length(ctx->evp_cipher))) != 1
	 || !pem_get_key(ctx->evp_cipher, key, salt_iv, password)
	 || (ctx->text && bio_printf(ctx->bio, "-----BEGIN %s-----\n", ctx->text) < 0)
