...
{
	/* XXX should be a 128 bits counter */
...
int sha512_224_final(uint8_t *md, struct sha512_ctx *ctx)
{
	...
	for (int i = 0; i < 3; ++i)
		be64enc(&md[i * 8], ctx->h[i]);
	be32enc(&md[3 * 8], ctx->h[3] >> 32);
	return 1;
}

int sha512_224(const uint8_t *data, size_t len, uint8_t *md)
{
	struct sha512_ctx ctx;
	if (!sha512_224_init(&ctx)
	 || !sha512_224_update(&ctx, data, len)
	 || !sha512_224_final(md, &ctx))
		return 0;
	return 1;
}
...
int sha512_256(const uint8_t *data, size_t len, uint8_t *md)
{
	struct sha512_ctx ctx;
	if (!sha512_256_init(&ctx)
	 || !sha512_256_update(&ctx, data, len)
	 || !sha512_256_final(md, &ctx))
		return 0;
	return 1;
}
