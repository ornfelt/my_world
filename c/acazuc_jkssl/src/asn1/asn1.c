                             ...
                             uint32_t type, size_t st_size)
{
	...
	object->type = type;
	refcount_init(&object->refcount, 1);
	return object;
}
...
{
	return refcount_inc(&object->refcount);
}
...
void asn1_constructed_free(struct asn1_constructed *constructed)
{
	if (!constructed)
		return;
	if (refcount_dec(&constructed->object.refcount))
		return;
                          ...
                          size_t *size)
{
	...
	if (size)
		*size = constructed->objects_count;
	return 1;
}
...
struct asn1_primitive *asn1_primitive_new(enum asn1_class class,
                                          uint32_t type)
{
	return asn1_object_new(class, 0, type,
...
void asn1_primitive_free(struct asn1_primitive *primitive)
{
	if (!primitive)
		return;
	if (refcount_dec(&primitive->object.refcount))
		return;
	free(primitive->data);
}
                       ...
                       uint8_t *data, size_t *size)
{
	...
	if (*size > primitive->size)
		*size = primitive->size;
	memcpy(data, primitive->data, *size);
	return 1;
}
...
void asn1_integer_free(struct asn1_integer *integer)
{
	if (!integer)
		return;
	if (refcount_dec(&integer->object.refcount))
		return;
	free(integer->data);
	free(integer);
}
...
void asn1_oid_free(struct asn1_oid *oid)
{
	if (!oid)
		return;
	if (refcount_dec(&oid->object.refcount))
		return;
	free(oid->values);
	free(oid);
}
...
void asn1_null_free(struct asn1_null *null)
{
	if (!null)
		return;
	if (refcount_dec(&null->object.refcount))
		return;
	free(null);
}
...
void asn1_bit_string_free(struct asn1_bit_string *bitstring)
{
	if (!bitstring)
		return;
	if (refcount_dec(&bitstring->object.refcount))
		return;
	free(bitstring->data);
	free(bitstring);
}
...
void asn1_string_free(struct asn1_string *string)
{
	if (!string)
		return;
	if (refcount_dec(&string->object.refcount))
		return;
	free(string->data);
	free(string);
}
...
int asn1_string_set0(struct asn1_string *string, char *data, size_t size)
{
	free(string->data);
	string->data = data;
	string->size = size;
...
void asn1_time_free(struct asn1_time *time)
{
	if (!time)
		return;
	if (refcount_dec(&time->object.refcount))
		return;
	free(time);
}
...
void asn1_octet_string_free(struct asn1_octet_string *octetstring)
{
	if (!octetstring)
		return;
	if (refcount_dec(&octetstring->object.refcount))
		return;
...
void asn1_boolean_free(struct asn1_boolean *boolean)
{
	if (!boolean)
		return;
	if (refcount_dec(&boolean->object.refcount))
		return;
	free(boolean);
}
