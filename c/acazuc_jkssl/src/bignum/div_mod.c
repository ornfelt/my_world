                   ...
                   struct bignum_ctx *ctx)
{
	...
	while (1)
	{
		if (!bignum_ulshift1(r, r, ctx))
			goto end;
		if (__bignum_is_bit_set(a, i)
		 && !__bignum_set_bit(r, 0))
			goto end;
		if (bignum_ucmp(r, b) >= 0)
		{
			if (!bignum_usub(r, r, b, ctx))
				goto end;
			if (!__bignum_set_bit(q, i))
				goto end;
		}
		if (!i)
			break;
                   ...
                   struct bignum_ctx *ctx)
{
	...
	bignum_init(&tmp);
	a_negative = __bignum_is_negative(a);
	b_negative = __bignum_is_negative(b);
	...
	if (dv)
		__bignum_set_negative(dv, a_negative != b_negative);
	if (rm)
	{
		if (a_negative && b_negative)
		{
			__bignum_set_negative(rm, 1);
		}
		else if (a_negative || b_negative)
		{
			__bignum_set_negative(rm, 0);
			__bignum_set_negative(&tmp, 0);
			if (!__bignum_is_zero(rm)
			 && !bignum_sub(rm, &tmp, rm, ctx))
			{
				ret = 0;
				goto end;
			}
			__bignum_set_negative(rm, b_negative);
		}
	}
end:
	bignum_clear(&tmp);
