...
mod test {
    ...
    fn verify_reconnection_proof() {
        ...
        for line in contents.lines() {
            ...
            let client_data = ReconnectData::from_be_hex_str(line.next().unwrap());
            let server_data = ReconnectData::from_be_hex_str(line.next().unwrap());
            ...
            assert_eq!(
                ...
                "username: {}\nclient_data: {}\nserver_data: {}\nsession_key: {}\n,expected: {}\n",
                username,
                client_data.as_be_hex_string(),
                server_data.as_be_hex_string(),
                session_key.as_be_hex_string(),
                expected.as_be_hex_string(),
            );
        }
    }
    ...
    #[test]
    #[allow(non_snake_case)]
    fn verify_split_s_key() {
        let contents = include_str!("../tests/srp6_internal/calculate_split_s_key.txt");
        for line in contents.lines() {
            let mut line = line.split_whitespace();
    ...
    fn verify_session_key() {
        let contents = include_str!("../tests/srp6_internal/calculate_server_session_key.txt");
    ...
    #[test]
    fn verify_xor_hash() {
        let contents = include_str!("../tests/srp6_internal/calculate_xor_hash.txt");
        ...
        for line in contents.lines() {
            ...
            let actual = calculate_xor_hash(&large_safe_prime, &generator);
