...
///
/// The minimum is always 4 bytes because the size is 2 bytes
...
///
/// The maximum is always 5 bytes because the size is 3 bytes
/// and the opcode is 2 bytes.
///
...
mod test {
    ...
    #[test]
    fn server_helper() {
        let contents = include_str!("../../tests/encryption/wrath_server.txt");
    ...
    #[test]
    fn client_helper() {
        let contents = include_str!("../../tests/encryption/wrath_client.txt");
        ...
        for line in contents.lines() {
            ...
            let header = enc.read_and_decrypt_client_header(data.as_slice()).unwrap();

            dec.write_encrypted_client_header(data.as_mut_slice(), size, opcode)
                .unwrap();
    ...
    fn verify_encrypt_and_decrypt() {
        ...
        for line in contents.lines() {
            ...
            let expected_server = hex_decode_be(line.next().unwrap());
